...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun c -> fun d -> d * c) 3 9 |> should equal 27

    [<Test>]
    let ``13 You can write a function as a one-liner (Part 2).`` () =
        (fun f g -> f + g) 9 8 |> should equal 17
    ...
    let ``20 Functions have types`` () =
        ...
        let b r = 50.0 / r
        a "friendly" "yes" |> should be ofType<string>
        b 5.0 |> should be ofType<float>
...
*)
    ...
    *)
    ...
    let ``34 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows noise = f "cow" noise + ", de gozaru"  // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
