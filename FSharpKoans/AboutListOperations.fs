        ...
        let length (xs : 'a list) : int =
            let rec doit xs acc =
                match xs with 
                  |[] -> acc 
                  |_ ::t -> doit t (acc+1)
            doit xs 0    // write a function to find the length of a list
        ...
        let rev (xs : 'a list) : 'a list =
            let rec doit xs acc =
                match xs with 
                 |[] -> acc 
                 |h::t -> doit t (h::acc)
            doit xs []    // write a function to reverse a list here.
        ...
        let map (xs : int list) : int list =
            let rec doit xs acc =
                match xs with 
                 |[] -> acc 
                 |h::t -> doit t (h + 1::acc)
            doit (List.rev xs) []  // write a function which adds 1 to each element
        ...
        let map (xs : int list) : int list =
            let rec doit xs acc =
                match xs with 
                 |[] -> acc 
                 |h::t -> doit t (h * 2::acc)
            doit (List.rev xs) [] // write a function which doubles each element
        ...
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec doit xs acc =
                match xs with 
                 |[] -> acc 
                 |h::t -> doit t ((f h)::acc)
            doit (List.rev xs) []  // write a map which applies f to each element
                ...
                match xs with 
                 ...
                 |h::t -> 
                         match f h with 
                          |true -> doit t (h ::acc)
                          | _ -> doit t (acc)
            doit (List.rev xs) []  // write a function which filters based on the specified criteria
                ...
                match xs with 
                 ...
                 |h::t -> 
                         match h%2=0 with 
                          |true -> doit t acc
                          |_ -> doit t (h ::acc)
            doit (List.rev xs) [] // write a function to filter for odd elements only.
...
*)
        ...
        let fold initialState xs =
            let rec doit xs sum =
                match xs with 
                 |[] -> sum
                 |h::t ->  doit t (h+sum)
            doit xs (initialState) // write a function to do what's described above
        ...
        let fold initialState xs =
            let rec doit xs sum =
                match xs with 
                 |[] -> sum
                 |h::t ->  doit t (h*sum)
            doit xs (initialState)// write a function to multiply the elements of a list
        ...
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
             let rec doit xs acc =
                 match xs with 
                  |[] -> acc
                  |h::t ->  doit t (f acc h)
             doit xs (initialState) // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
                ...
                match xs with 
                 ...
                 |h::t -> 
                         match f h with 
                          |true -> true
                          |_ -> doit t
            doit xs// Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec doit xs first second =
                match xs with 
                 |[] -> first,second
                 |h::t -> 
                         match f h with 
                          |true -> doit t (h ::first)second
                          | _ -> doit t first (h::second)
            doit (List.rev xs) [] []// Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec doit index acc =
                match index < n with 
                 |false -> List.rev acc
                 |_ ->  doit (index+1) (f index::acc)
            doit 0 []// Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
                ...
                match xs with 
                 ...
                 |h::t -> 
                         match p h with 
                          |true -> Some h
                          |_ -> doit t
            doit xs // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
                ...
                match xs with 
                 ...
                 |h::t -> 
                         match p h with 
                          |Some a -> Some a
                          |None-> doit t
            doit xs // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
                ...
                match xs with 
                 ...
                 |h::t -> 
                         match p h with 
                          |Some a -> doit t (a::acc)
                          |None -> doit t acc
            doit(List.rev xs) []// Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
             let rec doit idx xs acc  =
                 match xs with 
                  |[] -> List.rev acc
                  |h::t -> doit (idx+1) t (f (idx) h::acc)
             doit 0 xs []// Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
